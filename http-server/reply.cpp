...
boost::asio::const_buffer toBuffer(Reply::Status status)
{
	switch (status) {
	case Reply::Ok:
...
std::vector<boost::asio::const_buffer> Reply::toBuffers()
{
	...
	// текущий ответ в буфер
	buffers.push_back(statusStrings::toBuffer(status));

	// проходим по всем headers и засовываем их тоже в буфер
...
std::string toString(Reply::Status status)
{
	switch (status) {
	case Reply::Ok:
		return ok;
